...
function drawTrianguloTomada(ctx, cx, cy, sizePx, fillMode, stroke, lw) {
  ...
  function mount(root, ctx) {
    ...
    function paint() {
      ...
      const wagoLocaisAtivos = hasWagoFilter
        ? (projeto.lastAnalise?.wago?.locais || []).filter((l) => {
            ...
            if (selectedWago.pontoKey) {
              const key = (l.pontoIds || []).slice().sort().join("|") || `xy:${l.x},${l.y}`;
              return key === selectedWago.pontoKey;
            }
    ...
    function renderSide() {
      ...
      const wagoPorPonto = (() => {
        ...
        wagoLocais.forEach((l) => {
          ...
          if (!nome && ids.length) {
            nome = ids
              .map((id) => {
                const pt = (projeto.points || []).find((p) => p.id === id);
                return pt ? labelPonto(pt) : "";
              })
              .filter(Boolean)
              .join(" · ");
          }
            ...
            ${[2, 3, 5, 10]
              .map((pol) => {
                ...
                const active =
                  ...
                  !selectedWago.pontoKey &&
                  Number(selectedWago.size) === pol;
                return `<button type="button" class="pe-wago-chip ${active ? "active" : ""}" data-wago-size="${pol}">
                  <strong>${pol}P</strong> <span>${q} un</span>
                </button>`;
              })
              .filter(Boolean)
              .join("")}
      ...
      analysis.querySelectorAll(".pe-wago-chip[data-wago-size], .pe-wago-btn[data-wago-size]").forEach((el) => {
        el.onclick = () => {
          const size = Number(el.dataset.wagoSize);
          selectedCircuitId = null;
          if (selectedWago && !selectedWago.localId && !selectedWago.pontoKey && Number(selectedWago.size) === size) {
            selectedWago = null;
          } else {
            selectedWago = { size };
          }
      ...
      analysis.querySelectorAll(".pe-wago-ponto-head[data-wago-ponto], .pe-wago-ponto-link[data-wago-ponto]").forEach((el) => {
        el.onclick = (ev) => {
          ...
          paint();
          renderSide();
          const g = wagoPorPonto.find((x) => x.key === pontoKey);
          ctx.toast?.(
            selectedWago
              ? `${g?.nome || "Ponto"} · ${g?.qtdTotal || 0} Wago(s)`
              : "Seleção de Wago limpa"
          );
        };
      });
      analysis.querySelectorAll(".pe-wago-loc[data-wago-local], tr.pe-wago-row[data-wago-local]").forEach((el) => {
        el.onclick = (ev) => {
          ...
          if (selectedWago?.localId === localId) {
            ...
            const pontoKey = (loc?.pontoIds || []).slice().sort().join("|") || undefined;
            selectedWago = { size: loc?.size, localId, pontoKey };
          }
          paint();
          renderSide();
          ...
          ctx.toast?.(
            selectedWago
              ? `${loc?.pontoNome || loc?.label || "Ponto"} · Wago ${loc?.size}P${loc?.papel ? ` · ${loc.papel}` : ""} · ${circs || "—"}`
              : "Seleção de Wago limpa"
          );
        };
      });
